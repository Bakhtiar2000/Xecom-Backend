...
export class OrderService {
    ...
    // ------------------------------- Place Order -------------------------------
    public async placeOrder(customerId: string, placeOrderDto: PlaceOrderDto) {
        const { addressId, street, postalCode, thanaId, addressType, saveAddress, notes, couponCode } = placeOrderDto;
        ...
        if (!addressId && street && thanaId) {
            ...
            const newAddress = await this.orderRepository.createAddress({
                ...
                thanaId,
                addressType,
                userId: saveAddress ? customerId : null, // Link to user if saveAddress is true
            });
    ...
    public async getAllOrders(
        ...
        const [orders, total] = await Promise.all([
            ...
            this.orderRepository.countAllOrders(status, searchTerm, deliveredFrom, deliveredTo, customerId),
        ]);
    ...
    // ------------------------------- Get Single Order -------------------------------
    public async getSingleOrder(customerId: string, orderId: string) {
    ...
    public async updateOrderStatus(
        ...
        if ((finalStatuses as readonly OrderStatus[]).includes(existingOrder.status)) {
            throw new HttpException(
